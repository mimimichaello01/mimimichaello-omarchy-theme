...
- тёмная палитра ATLAS и набор обоев;
- единые цвета терминалов, VS Code, Obsidian, Yazi и micro;
- оформление fastfetch, starship и подсветки Zsh;
...
Установщик сохраняет текущие пользовательские файлы перед изменением. Настройки
VS Code и `.zshrc` корректируются точечно: существующая конфигурация редактора,
плагины и раскладка панели Omarchy не удаляются.
...
```bash
~/.config/omarchy/themes/mimimichaello-omarchy/install/mimimichaello
```

Команда применит тему, настройки Hyprland, уведомления, Lock screen, GTK,
шрифты и цвета приложений. Системный Plymouth при этом не изменяется.

### Вариант 2: полная установка с Plymouth

```bash
~/.config/omarchy/themes/mimimichaello-omarchy/install/mimimichaello --with-plymouth
```
...
```bash
~/.config/omarchy/themes/mimimichaello-omarchy/install/mimimichaello --status
```

Вернуть состояние, сохранённое перед первым запуском установщика:

```bash
~/.config/omarchy/themes/mimimichaello-omarchy/install/mimimichaello --revert
```
